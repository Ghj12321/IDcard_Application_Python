...
def ROI1():  # argc, argv
    ...
    while True:
        ...
        img_gray2 = cv2.adaptiveThreshold(
            img_gray2, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2
        )
        img_edge2 = cv2.Canny(img_gray2, 60, 140, 3)
        ...
        for contour in contours:
            ...
            if cv2.contourArea(contour) > (300 * 189 * 0.5):
                ...
                if abs(1.6 - float(rect[2]) / rect[3]) <= 0.1:
                    ...
                    # 주민등록증에 해당하는 윤곽선 표시
                    cv2.rectangle(img_roi, rect, (0, 0, 255), 2)

                    for contour2 in contours2:
                        if cv2.contourArea(contour2) > (100 * 125 * 0.5):
                            rect2 = cv2.boundingRect(contour2)

                            if abs(0.8 - float(rect2[3]) / rect2[2]) <= 0.15:
                                ...
                                if not contourFound:
                                    ...
                                    if Yees >= 1:
                                        ...
                                        if image_processing_completed == True:
                                            x, y, w, h = rect
                                            captured = img_result[
                                                y : y + h, x : x + w
                                            ].copy()
...
def ROI2():  # argc, argv
    ...
    while True:
        ...
        img_gray2 = cv2.cvtColor(img_roi, cv2.COLOR_BGR2GRAY)
        img_gray2 = cv2.GaussianBlur(img_gray2, (5, 5), 0)
        img_edge2 = cv2.Canny(img_gray, 60, 140, 3)
...
def login():
    ...
    # MySQL 데이터베이스 연결
    connector = MySQLConnector("192.168.0.56", "user1", "1234", "identification")
    connector.connect()
...
def signup():
    if (
        "userid_checked" not in session
        or not session["userid_checked"]
        or session["is_duplicate"]
    ):
        # 중복 확인이 이루어지지 않았거나 중복된 아이디인 경우
        return (
            jsonify(
                {
                    "error": "중복확인을 하지 않았거나 중복된 아이디입니다.",
    ...
    return jsonify({"message": "회원가입이 완료되었습니다.", "signup_success": True})
...
def image_save():
    ...
    if isinstance(captured, np.ndarray):
        ...
        print("captured 변수가 NumPy 배열이 아닙니다.")

    # 이미지 저장 후 메인 메뉴로 리디렉션
...
def check_status():
    # 이미지 처리 상태를 반환
    global image_processing_completed
    status = jsonify({"completed": image_processing_completed})
    image_processing_completed = False  # 이미지 처리 상태를 반환한 후에는 변수를 초기화
    return status
...
@app.route("/reset_recognition", methods=["GET"])
def reset_recognition():
    global identity_recognized
    identity_recognized = False  # 신분증 인식 여부를 초기화
    return jsonify({"success": True})


@app.route("/hi", methods=["GET", "POST"])
def hi_world():
    global captured, login_userid
    text_list.clear()
...
def check_recognition():
    global identity_recognized
    # 신분증 인식 여부 확인 로직
    # 예를 들어, 신분증 인식 여부를 저장하는 변수가 `identity_recognized`라고 가정합니다.
    if identity_recognized:  # 이 변수는 신분증이 인식되었는지 여부를 나타냅니다.
